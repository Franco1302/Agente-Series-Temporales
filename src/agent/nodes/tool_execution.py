...
def _current_turn_index(state: AgentState) -> int:
    ...
    Se usa como metadato informativo en `session_facts["by_param"]` para que se
    pueda saber cuándo se introdujo un parámetro. No interviene en la lógica de
    herencia: si dos tools establecen el mismo parámetro, gana la última.
    """
    from langchain_core.messages import HumanMessage  # local import: evita ciclo
